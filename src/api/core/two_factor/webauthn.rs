...
use webauthn_rs::{
    ...
    proto::{
        AuthenticatorAssertionResponseRaw, AuthenticatorAttestationResponseRaw, Credential, PublicKeyCredential,
    ...
    fn with_u2f(u2f: bool) -> Self {
        ...
        Self {
            rpid: reqwest::Url::parse(&domain)
                ...
                .flatten()
                .unwrap_or_default(),
            appid: format!("{}/app-id.json", &domain),
            url: domain,
            u2f,
        }
    }
    fn load(self) -> Webauthn<Self> {
        Webauthn::new(self)
    }
}
...
fn generate_webauthn_challenge(data: JsonUpcase<PasswordData>, headers: Headers, conn: DbConn) -> JsonResult {
    ...
    let (challenge, state) = WebauthnConfig::with_u2f(false).load().generate_challenge_register_options(
...
pub fn generate_webauthn_login(user_uuid: &str, conn: &DbConn) -> JsonResult {
    ...
    let config = WebauthnConfig::with_u2f(false);
    let ext = RequestAuthenticationExtensions::builder().appid(config.appid.clone()).build();
    let (response, state) = config.load().generate_challenge_authenticate_options(creds, Some(ext))?;
...
pub fn validate_webauthn_login(user_uuid: &str, response: &str, conn: &DbConn) -> EmptyResult {
    ...
    let (cred_id, auth_data) = WebauthnConfig::with_u2f(use_u2f).load().authenticate_credential(&rsp, &state)?;
